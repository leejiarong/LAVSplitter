...
STDMETHODIMP CLAVSplitter::CompleteInputConnection()
{
  ...
  //Bug: M4A might hang in SetPosition or no sound
  ...
  if (//_wcsicmp(extension, L".m4a") == 0 ||	//1.Check extension of file name
	  !videoStream)								//2.Check if has video stream(use it!)
	  return E_FAIL;
